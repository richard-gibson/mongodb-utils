...
class FilterSpec : ShouldSpec({
  ...
  context("Filter nor") {
    should("render one nested filter") {
      nor(streetAddress.name eq "name") bsonShouldMatch Filters.nor(
        Filters.eq("address.street.name", "name")
      )
    }
